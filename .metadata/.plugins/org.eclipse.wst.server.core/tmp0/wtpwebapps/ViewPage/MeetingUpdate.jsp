...
<script type="text/javascript">
	$(document).ready(function()   // 페이지가 로드됐을 때 제이쿼리 실행?
    {
        ...
        $("#submitBtn").click(function()
        {
			...
			if( $("#title").val()=="" || $("#content").val()=="" || $("#fee").val()=="" 
					|| $("#beeTime1").val()=="" || $("#beeTime2").val()==""
					|| $("#location").val()==""
					|| $("#min").val()==""
					|| $("#max").val()=="")	
			{
...
<body>
	...
	<form action="beeupdate.action" method="post" id="beeForm">
		...
		<div class="upper">
			...
			</select> 모임 제목<input type="text" id="title" name="title"  value="${bee.title }"> 주최자 : 글작성자닉네임 받아오기
		</div>
		<div class="bottom">
			<div class="intro">
				<h3>모임소개</h3>
				<textarea cols="50" rows="10" id="content" name="content" >${bee.content }</textarea>
			</div>
			...
			<div class="bottom2">
				<div class="left">
					...
					<br></br> 참가비 <input type="text" id="fee" name="fee" value="${bee.fee }"> 
					<br> 모임일 <input type="date" id="beeTime1" name="beeTime1" value="${fn:substring(bee.beeTime,0,10) }"> 
					<%-- <br>
					${fn:substring(bee.beeTime,0,10) }<br>
					${fn:length(bee.beeTime) }<br>
					${fn:substring(bee.beeTime,11,16) }<br>
					${bee.beeTime} --%>
					<br> 모임시간<input type="time" id="beeTime2" name="beeTime2" value="${fn:substring(bee.beeTime,11,16) }"><br>
					<%-- <br> 모임시간<input type="time" id="beeTime2" name="beeTime2" value="${fn:substring(bee.beeTime,12,5) }"><br> --%>
					
					모임장소<input type="text" id="location" name="location" value="${bee.location }"><br> 
					인원<input type="number" id="min" name="min" value="${bee.min }"> - <input type="number" id="max" name="max" value="${bee.max }"><br>
			...
			<div class="enroll">
				<button id="submitBtn" type="button" class="btn btn-warning">수정</button>
